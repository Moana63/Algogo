...
from proba_2mers import indexation
...
def write_file(filein, fileout):
    ...
    list_keys = sorted([(key[:9], key[9:]) for key in index.keys()])
    list_keys_sorted = [f'{a}{b}' for (a, b) in list_keys]
    max_memory = get_memory()
    with open(fileout, "w") as out_file:
        for key in list_keys_sorted:
