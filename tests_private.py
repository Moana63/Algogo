...
from main import write_ordered_kmers, compress_naive
...
FUNC_TEST: list[Callable] = [compress_naive, write_ordered_kmers]
...
@timer
def my_tester(func: Callable, input: str, output: str) -> None:
    return {'memory': func(input, output), 'file': output}
...
if __name__ == "__main__":
    ...
    for func in FUNC_TEST:
        func_ret: list[tuple] = [
            my_tester(func, INPUT[i], OUTPUT[i]) for i, _ in enumerate(INPUT)]
        ...
        ax1.plot(x, serie_time, label=func.__name__)
        ax2.plot(x, serie_memory, label=func.__name__)
        ax3.plot(x, serie_du, label=func.__name__)
